...
implementation
 ...
 procedure nomforms;
 var i,j:integer;st,res:tstringlist;noms:tnominit;
 begin
   writeln('ttttttttttttttttttttttemp');
   noms:=tnominit.create('sanatuus.csv','nmids.csv');

  for i:=0 to 33 do begin writeln('<li>','ilo'+noms.lmmids[0,i]+(noms.sijat[i].ending));end;
  exit;
  st:=tstringlist.create;
  res:=tstringlist.create;
  st.loadfromfile('esim.tmp');
...
procedure riimei;
var riimii:triimitin;verbit:tverbit;nominit:tnominit;
begin
 //ILKO;
...
var //hakunen:tvhaku;
     ha,i,ohits,hs,hm:integer;hakusana:string;
     hakunen:thakunen;
     aresulist,aresulist2,HAKULISTA,uushAKUlista:tstringlist;
     onjolist:tlist;
begin
   //exit;
