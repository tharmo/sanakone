...
vAR curs,togos:ARRAY[0..64000] OF WORD;
  i,CURLEV,togo:WORD;curtag:ttag;ind:string;
  stakki:tlist;
begin
 try
  try
 counter:=0;
  ...
  togos[curlev]:=1;
  curtag:=ele;
 except writeln('!!!start');raise;end;
  while curlev>=0 do
  begin
    try
    res(_onelinexml(curtag,ind));
    curs[curlev-1]:=curs[curlev-1]+1;
    togos[curlev-1]:=togos[curlev-1]-1;
    except writeln('!!!fail element list');raise;end;
    try
    ...
    begin  //unhandled stuff at this level
      ...
      except writeln('<li>!!!next');raise;end;

    end else
    begin  //bactrack
      while togos[curlev-1]=0 do
      begin
        try
         curlev:=curlev-1;
        ind:=copy(ind,3,9999);
     ...
     except writeln('<li>!!!furtherx');raise;end;
    if curlev<1 then break;
  end;
 finally
    stakki.free;
