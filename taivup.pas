...
type tmatchmaker=class(tobject)
   //tothits,lkahits, vokhits,avhits,stem                          4,v0,1,
     hits:tstringlist;
     cuts:word;
...
slist.loadfromfile('nominit/vsijat.lst');
...
var winfo,parts:tstringlist;ofi,ifi:textfile;
     ...
     mux:word;
     avlen,tc:byte;
     alkkon,voksoi,xtra:ansistring;

    begin
    ends:=tstringlist.create;
    winfo:=tstringlist.create;
    parts:=tstringlist.create;
     assign(ofi,'verbstosort.csv');  //selected set of wrds
     rewrite(ofi);
    ...
    begin
     ...
     xtra:='';
     mymid:=curlka.mids[1];
     voksoi:=ifs(parts[2][length(parts[2])]='a','0','1');//+parts[2][length(parts[2])];
     ...
     begin
             ...
             if parts[0]='52' then xtra:=taka(parts[2][length(parts[2])-1]);
                  //lopvoks:=taka(parts[2][length(parts[2])-1]);
               color:='green';
               //hioa                          4                   1     0  1 -> hi
               stem:=taka(copy(parts[2],1,length(parts[2])-length(mymid+av+'a')));

          if av='' then     writeln('<li>stem:',stem,'/av:',av,'/ah:',ah,'/',alkkon);

               //writeln('<li>',stem, '.',lopvoks,' ',hyphenfi(stem+lopvoks,tavus));
               if av='' then
               begin
                 for j:=length(stem) downto length(stem)-2 do if j>1 then if pos(stem[j],vokaalit)<1 then begin av:=stem[j];ah:=av;delete(stem,j,1);break;end;
                 writeln(parts[0]+','+xtra+ansireversestring(lopvoks)+','+av+','+ah+','+voksoi+','+ansireversestring(stem)+','+alkkon);

               end;
               //jos kahdessa vikassa konsonantti, tee siit‰ astevaihtelija .. mit‰ jos oli v‰livokaali?
               alkkon:='';
               if stem='' then begin stem:=av;av:='';ah:='';end;
               if av=ah then av:='a';
               while (stem<>'') and (pos(stem[1],vokaalit)<1) do begin alkkon:=alkkon+stem[1];delete(stem,1,1);end;
               //stem:i/av:a/ah:h/
               winfo.commatext:=parts[0]+','+xtra+ansireversestring(lopvoks)+','+av+','+ah+','+voksoi+','+ansireversestring(stem)+','+alkkon;
         if av='a' then        writeln('<li>',laskuri,':',winfo.commatext+' ! ',line);
                 writeln(ofi,winfo.commatext);
                 //writeln('<li>',laskuri,':',winfo.commatext+' ! ',line);
                //continue;
   end
   ...
   begin
          ...
          if parts[0]='67' then xtra:=parts[2][length(parts[2])-1];
            ...
            while (stem<>'') and (pos(stem[1],vokaalit)<1) do begin alkkon:=alkkon+stem[1];delete(stem,1,1);end;
            if av=ah then av:='a';
            winfo.commatext:=trim(parts[0]+','+xtra+ansireversestring(lopvoks)+','+av+','+ah+','+voksoi+','+ansireversestring(stem)+','+alkkon);
            writeln(ofi,winfo.commatext);
            continue;
...
POISTETTU listall VAHVAT lopusta

for  i:=tavus.count-1 downto 1  do winfo.add(ansireversestring(tavus[i]));

xpart:=copy(parts[0],length(stem)+1);
if tavus.count<2 then writeln('!=============',line,'**');
///VAHVAAA
xpart:=copy(xpart,1,length(xpart)-length(mymid)-1);
hyphenfi(stem,tavus);
...
//   av:='';ah:='';voksoi:='';newid:='';voksoi:='';
winfo.commatext:=parts[0]+','+xtra+ansireversestring(lopvoks)+','+av+','+ah+','+voksoi;
//for i:=0 to tavus.count-1  do winfo.add(tavus[i]);
for  i:=tavus.count-1 downto 1  do winfo.add(ansireversestring(tavus[i]));
//winfo.commatext:=newid+','+lopvoks+','+av+','+ah+','+voksoi+',';
//for i:=0 to tavus.count-1  do winfo.add(tavus[i]);
writeln(ofi,winfo.commatext, '  ');
continue;
form:='';
...
POISTETTU listall HEIKOT lopusta
hyphenfi(stem,tavus);
if tavus.count<2 then writeln('!=============',line,'**');
...
//!!    winfo.commatext:=newid+','+lopvoks+','+     inttostr(length(ast.f+ast.t))+','+ifs(ast.t+ast.f='','_,'+av,av+','+ah)+','+VOKSOI+','+inttostr(tavus.count-1);
//   av:='';ah:='';voksoi:='';newid:='';voksoi:='';newid:='';
winfo.commatext:=trim(parts[0]+','+xtra+ansireversestring(lopvoks)+','+av+','+ah+','+voksoi);
// for i:=0 to tavus.count-1  do winfo.add(trim(tavus[i]);
for  i:=tavus.count-1 downto 1  do winfo.add(ansireversestring(trim(tavus[i])));
...
//winfo.commatext:=newid+','+ifs(ast.f='',av,ast.f)+','+ifs(ast.f='',av,ast.t)+','+inttostr(tavus.count);
//winfo.commatext:=newid+','+inttostr(length(ast.f+ast.t))+','+av+ah+','+inttostr(tavus.count);
//   for i:=0 to tavus.count-2 do winfo.add(trim(tavus[i]));
writeln(ofi,winfo.commatext);
