...
BEGIN
 ...
 begin
   ...
   if pos(hakulist[i][j],'aou')>0 then begin write('#',hakulist[i][j]);takav:=true;end;
//   if takav then hakulist[i]:='0'+hakulist[i] else hakulist[i]:='1'+taka(hakulist[i]);
   writeln('<h1>',hakulist[i],takav,'</h1>');
  end;
END;
...
begin
  ...
  begin
      ...
      hyphenfi(hitw,hittavut);   //tavus pit‰is olla k‰‰nt.j‰rj. ja pakattuina ttavuiksi. myˆhemmin
      //writeln('<li>X ',hits[i]);
