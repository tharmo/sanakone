...
procedure haehaku(hakulist:tstringlist;hakufile:string);
VAR I,j:WORD;takav:boolean;
BEGIN
 ...
 begin
   ...
   if pos(hakulist[i][j],'aou')>0 then takav:=true;
   if takav then hakulist[i]:='0'+hakulist[i] else hakulist[i]:='1'+taka(hakulist[i]);
  end;
END;
...
function matchmid(haku,mymid:ansistring;var cut:integer;var alku,lopkon:ansistring):boolean;
begin
  cut:=0;lopkon:='';
  ...
  if haku<>'' then if pos('_',mymid)>0 then  mymid[pos('_',mymid)]:=haku[length(haku)];//whatever goes, and s forgotten
  result:=matchendvar(haku,mymid,alku);
  writeln('<li>',haku,' <b style="color:red">[',mymid,']</b>=',alku,'///',lopkon,'</li>')
...
var ha,lu,si,ko,vo,av,sa,le,salu,i:word;
 ...
 cut,mycut,muoto:integer;
 avvahvat,sopii,etuko:boolean;
begin
 ...
 begin
   ...
   //for salu:=0 to o
   //matchsijat
   writeln('<h2>trysana:',hakusana,'</h2><li>',sikalauma.commatext,'</ul>');
   ...
   begin
    ...
    // if muoto<>5 then continue;
     writeln('<li>SIKA:',sika,'#',muoto,'::',sikalauma.commatext,'</li><li>');
     for lu:=0 to 48 do writeln('|',lmmids[lu,muoto]);
     writeln('<li> (',lu,'#',muoto,'/',sika,') <hr>') ;
     writeln('<ul>');
     ...
     begin
       ...
       writeln('<h4>lka:',lu+1,'#',muoto,'  ',sika,'/',amid,':',paatteet[muoto],'</h4><ul>');
       lopkon:='';


       if matchmid(sika,lmmids[lu,muoto],cut,lka_a,lopkon) or FALSE then
       begin //LKA
         //1samma|1sammalee     9          3
         amid:=copy(sika,length(lka_a)+length(lopkon)+1);
         writeln('<li>OK:<b>',lka_a,'</b>|','.',alop,'</b> ',luoks[lu].ekakon,'...',luoks[lu].vikakon,'//',lopkon,'\\\<b>',alop,'</b>',hakusana,'--',cut) ;
         ...
         begin    //KON
           ...
           if (lopkon='')  or (lopkon=kons[ko].kon) then //or (matchendvar(lka_a,lopkon,kon_a)) then
              //if ((lopkon='') and (kons[ko].kon='_')) or matchendvar(lka_a,kons[ko].kon,kon_a) or false then
           begin  //KON
              writeln('<li>kon<b>:',lopkon,'/</b> '+kon_a,': ',kons[ko].ekavok,'..', kons[ko].vikavok ,' ') ;
              ...
              begin  //VOK
                ...
                begin  //VOK
                  ...
                  writeln('<li>vok:<b>[',avok,']</b>: ',vok_a,mycut);
                  ...
                  for av:=voks[vo].ekaav to voks[vo].vikaav do
                   ...
                   if trim(avok)='' then writeln('xxx<hr/>');
                  writeln('<ul>');
                  for av:=voks[vo].ekaav to voks[vo].vikaav do
                  begin
                    //avok:=voks[av].vok;
                    //for i:=1 to cut do if avok<>'' then begin cut:=cut-1;delete(avok,length(avok),1);end;
                    ...
                    begin   //AV
                      ...
                      aav:=ifs(avvahvat,avs[av].v,avs[av].h);
                      writeln('<li>AV:<b>[',avs[av].v,avs[av].h,'=',aav,']</b> ',av_a,'<ul>');
                      ...
                      // while (sopiva<>'') and (pos(sopiva[length(sopiva)],vokssliy)>0) do delete(sopiva,length(sopiva),1);
                       ...
                       if lu=0 then if sanat[sa]='el'
                       then writeln('<li>try:',' ',sanat[sa],'/',asana,'/',ansireversestring(av_a),sopii,(trim(av_a)='') ,av_a);
                      ...
                      begin  //SANA
                          ...
                          begin  //sana  oli lyhkäsempi kuin haku
                            ...
                            if sopii then  writeln('<li>HIT:XXX', ansireversestring(sanat[sa]),'.'
                             ,aav,avok,lopkon,amid,alop,'  ',luoks[lu].kot,' ',sa,paatteet[muoto],'//',asana,'//',av_a,'!');
                          end;
                          if not sopii then  continue;
                          writeln('<h3>HIT:', ansireversestring(sanat[sa]),'.',aav,avok,lopkon,amid,alop,'  ',luoks[lu].kot,' ',sa,paatteet[muoto]);
                          ...
                          writeln('</h3><li> _',avs[av].v,' ',avs[av].h,' ',voks[vo].vok,' ',kons[ko].kon,' ',lmmids[lu,muoto],' ',paatteet[muoto],muoto);
       ...
       writeln('<strike>',muoto,sika,lmmids[lu,muoto],'</strike> ');
       writeln('</ul>');
     end;
     writeln('</ul>');
   end;
   ...
   writeln('<h1>TULOS:',hakusana,'|',hakusanat[ha],'</h1>');
   writeln('<li>RES:',resus.commatext);

   tsekkaatavut(hakusana,etuko,resus,riimit);
   //for i:=0 to resus.count-1 do   begin asana:=copy(resus[i],2);   end;
     //writeln('<li>',ifs(resus[i][1]='-',asana,etu(asana)));end;
 end;
