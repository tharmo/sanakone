...
BEGIN
 ...
 if result then begin alku:=copy(sa,1,lendif);end;
 ...
 begin
  ...
  begin
     ...
     begin        //a ie  prev:e nyt:i  ie on dift
       ...
       //else if (isdifto(ch,chprev)) then //or  ((i>1) and (pos(w[i-1],vokaalit)>0)) then //kolm vok pÃ¤tks aina kaksi vikaa?                     //  as-il a a is | os
       begin
  ...
  //if alkkon<>'' then tavus[tavus.count-1]:=alkkon+tavus[tavus.count-1];  //ei nÃ¤in kun sanoja rakennellaan - alkukonsontit talletaan sanan tietoihin, ei ekaan tavuun
  //if alkkon<>'' then tavus[0]:=alkkon+tavus[0];  //ei nÃ¤in kun sanoja rakennellaan - alkukonsontit talletaan sanan tietoihin, ei ekaan tavuun
...
begin
  ...
  writeln('<hr>');
   ...
   writeln('<hr>keskijämät<table border="1">');
   for lu:=0 to 49 do
   begin
